...
if ($visiteur >= $level_access && $level_access > -1)
{
    ...
    function edit($mess_id)
    {
		...
		alertePost("edit", $url, $_REQUEST['titre']);
    ...
    function reply()
    {
        ...
        if (mysql_affected_rows() > 0) 
		{
			...
			/** Ajout module AlertPost */
			alertePost("reply", $link_post, $_REQUEST['titre']);
			/** Fin ajout module AlertPost. */
    ...
    function post()
    {
        ...
        echo "<br /><br /><div style=\"text-align: center;\">" . _MESSAGESEND . "</div><br /><br />";
		/** Ajout module AlertPost */
		alertePost("post", $url, $_REQUEST['titre']);
		/** Fin ajout module AlertPost. */
        redirect($url, 2);
        closetable();
    }
	...
	function alertePost($param, $url, $titre)
	{
		...
		$sql5 = @mysql_query("SELECT value FROM ".$table_pref." WHERE name='alerteActive'");
		list($alerteActive) = @mysql_fetch_row($sql5);
		
		$sql6 = @mysql_query("SELECT value FROM ".$table_pref." WHERE name='alertePostActive'");
		list($alertePostActive) = @mysql_fetch_row($sql6);
		
		$sql7 = @mysql_query("SELECT value FROM ".$table_pref." WHERE name='alerteReplyActive'");
		list($alerteReplyActive) = @mysql_fetch_row($sql7);
		
		$sql8 = @mysql_query("SELECT value FROM ".$table_pref." WHERE name='alerteEditActive'");
		...
		{			
			...
			$sql_niveau = mysql_query("SELECT value FROM ".$table_name ." WHERE name='niveauSel'");
			list($niveau) = mysql_fetch_row($sql_niveau);
			
			if ($niveau==0)
			{
				$sql_mails=mysql_query("SELECT mail FROM ".USER_TABLE);
			}
			if ($niveau>0&&$niveau<10)
			{
				$sql_mails=mysql_query("SELECT mail FROM ".USER_TABLE." WHERE niveau='" . $niveau . "'");
			}
			if ($niveau==11)
			{
				$sql_mails=mysql_query("SELECT mail FROM ".USER_TABLE." WHERE team>0");
			}
			if ($niveau==9)
			{
				$sql_mails=mysql_query("SELECT mail FROM ".USER_TABLE." WHERE niveau='9'");
			}
			if ($niveau==10)
			{
				$sql_userfor = mysql_query("SELECT value FROM ".$table_name ." WHERE name='userFor'");
	...
	function sendmail($mail, $titre_new_mess, $lien, $table_name){
		...
		$sql_sujet = mysql_query("SELECT value FROM ".$table_name ." WHERE name='sujet'");
		list($sujet) = mysql_fetch_row($sql_sujet);
		
		$sql_message = mysql_query("SELECT value FROM ".$table_name ." WHERE name='message'");
		list($message) = mysql_fetch_row($sql_message);
		
		$sql_user = mysql_query("SELECT value FROM ".$table_name ." WHERE name='sendUser'");
		list($send_user) = mysql_fetch_row($sql_user);
		
		$sql_titre = mysql_query("SELECT value FROM ".$table_name ." WHERE name='sendTitre'");
		list($send_titre) = mysql_fetch_row($sql_titre);
		
		$sql_url = mysql_query("SELECT value FROM ".$table_name ." WHERE name='sendUrl'");
	...
	function getTableName($param)
	{
		...
		switch($param)
		{
			case "post":return $nuked['prefix'] . "_alerteposte_pref";break;
			case "reply":return $nuked['prefix'] . "_alerteposte_pref_reply";break;
			case "edit":return $nuked['prefix'] . "_alerteposte_pref_edit";break;
		}
	}
